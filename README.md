...
Для запуску додатка в docker-контейнерах запустіть в корені проекту команду: `docker-compose up --build`.

*Проект орієнтовано буде остаточно запущений і готовий до роботи протягом 2-3 хвилин після завантаження всіх образів.*

Після запуску додатка будуть доступні такі URL:
- `http://localhost:8000` - для взаємодії з додатком (Guest service).
- `http://localhost:8001` - для керування сервісами (Controller service).
...
- Logrus 1.9.3 (github.com/sirupsen/logrus)
...
- **runner.bat, builder.bat** - файли для автоматичного запуску та компілингу (під Linux) мікросервісу.
...
**Змінні оточення:**
- `ID` - ідентифікатор екземпляру мікросервіса.
...
- `LOGS_DIR` - шлях для зберігання файлу логів мікросервісу в форматі *path/to/logs/*.


### Worker service

**1. Отримання задачі:** Воркер отримує і валідує задачу від Kafka з топіку `FindWords`, в якій передається:
- Посилання (`url`).
- Ідентифікатор запиту, до якого відноситься дане посилання (`request_id`).
- Кількість невдалих спроб обробки цієї задачі (`attempts`).
...
- Якщо перехід невдалий і кількість невдалих спроб дорівнює або більше 2-х разів, то URL додається в список `completed_urls`, але зі статусом `Fail`, та здійснюється завершення обробки.
- Якщо перехід вдалий, але сторінка не змогла завантажилась, то посилання також додається в список оброблених посилань, але зі статусом `Unreaded` і воркер також завершує обробку.

**4. Обробка документа:**
- Якщо сторінка завантажилась коректно, то потім із Redis дістаються дані запиту із списку `requests_data`, тобто ключові слова, які потрібно знайти, і чи потрібно фільтрувати посилання за доменом.
- Здійснюється парсинг посилань та створення нових задач:
    - Здійснюється пошук всіх посилань в документі.
    - Всі знайдені посилання фільтруються (перевіряючи чи є посилання в списку `completed_urls`), залишаючи тільки ті, що ще не були оброблені раніше.
...
- Здійснюється парсинг ключових слів, і обробляєме посилання разом із цими словами додається в список оброблених посилань `completed_urls` під статусом `Success`.

**5. Оповіщення агрегатора:** Якщо кількість відфільтрованих задач більше нуля, то обробка завершується, а якщо дорівнює нулю, то дане обробляєме посилання являється потенційно останім в цьому запиті, тому створюється і відсилається задача для агрегатора в `AggregateResult`.
...
Якщо кількість знайдених і оброблених посилань збігається, запит вважається завершеним. Інакше задача ігнорується.

**3. Агрегація даних:**
- Сервіс дістає результати обробки з `completed_urls`.
- Приводить їх до потрібного формату.
- Записує агреговані результати у базу даних.
